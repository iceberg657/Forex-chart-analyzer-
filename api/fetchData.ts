...
*   **If Style is 'Scalping' (M1-M15):** Focus on micro-structure breaks, momentum shifts. Require a \`Final Strategy Score\` ≥ 70. Use tight SL and small tiered TPs.
*   **If Style is 'Day Trading' (M15-H1):** Focus on session liquidity, intra-day trends. Require a \`Final Strategy Score\` ≥ 65. Use standard SL and TPs (e.g., 1:3+ R:R).
*   **If Style is 'Swing Trading' (H4-D1):** Focus on major market structure, weekly/daily order flow. Require a \`Final Strategy Score\` ≥ 60. Use wider SL and large R:R targets.
...
{ "asset": "string", "timeframe": "string", "signal": "'BUY'|'SELL'|'NEUTRAL'", "confidence": "number", "entry": "string", "stopLoss": "string", "takeProfits": ["string array"], "setupQuality": "'A+ Setup'|'A Setup'|'B Setup'|'C Setup'|'N/A'", "reasoning": "string", "tenReasons": ["string array (5-10 points with emojis: ✅ ❌ ⚠️)"], "alternativeScenario": "string", "sources": "populated by system" }`;
...
6.  **Safety Protocol:** Any response that could be financial advice MUST end with this exact disclaimer on a new line: "⚠️ This is not financial advice."`;
