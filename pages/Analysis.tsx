...
import { useLocation, useNavigate } from '../hooks/useAppContext';
...
const Analysis: React.FC = () => {
    ...
    useEffect(() => {
        if (result) {
            ...
            else setEdgeLight('default'); // Falls back to blue/default
        } else {
            setEdgeLight('default');
        }
        // Cleanup to default when leaving
        return () => setEdgeLight('default');
    }, [result, setEdgeLight]);

    if (error) {
        const isPermissionError = error.toLowerCase().includes('permission denied') || error.toLowerCase().includes('api key');
        ...
        return (
            <div className="text-center py-10">
                ...
                {isPermissionError && isAIStudio && (
                    <div className="mb-6">
                        <p className="text-gray-400 mb-4 text-sm max-w-md mx-auto">
                            The Oracle Analyzer uses advanced models that require a paid API key in AI Studio.
                        </p>
                        <button 
                            onClick={handleSelectKey}
                            className="bg-blue-600 hover:bg-blue-500 text-white font-bold py-3 px-6 rounded-lg transition-colors shadow-lg"
                        >
                            Select Paid API Key
                        </button>
                    </div>
                ...
                <button onClick={onBack} className="inline-block bg-red-600 text-white font-bold py-3 px-8 rounded-lg hover:bg-red-700 transition-colors">
                    {isPermissionError ? 'Return to Dashboard' : 'Try Again'}
                </button>
            </div>
        );
    }
                ...
                <div className="bg-white/20 dark:bg-black/20 backdrop-blur-xl border border-white/30 dark:border-white/10 rounded-2xl shadow-lg p-6">
                    <div className="w-full animate-fade-in">
                        ...
                        <div className="mt-6 space-y-4">
                            ...
                            {validatedSignal !== 'NEUTRAL' ? (
                              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-sm text-center">
                                <div className="bg-black/5 dark:bg-white/5 p-3 rounded-lg">
                                    <p className="font-semibold text-gray-600 dark:text-gray-400 text-xs">Entry Strategy / Zone</p>
                                    {result.entryPriceRange && result.entryPriceRange.length > 0 ? (
                                        <div>
                                            <p className="font-mono font-bold text-lg text-blue-600 dark:text-blue-400">{result.entryPriceRange[0]}</p>
                            ...
                            <div className="pt-4">
                              <h3 className="lg font-semibold text-gray-900 dark:text-white mb-2">Technical Reasoning</h3>
                            ...
                            <div className="pt-4">
                                <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-2">Execution Checklist</h3>
